...
class ConverterVC: UIViewController {
    ...
    func getTodayDate() {
        ...
        currentRatesDate = formatter.string(from: Date())
    }
    ...
    func getApiEcbConvertRates(data: ConversionData) {
        ...
        DispatchQueue.main.async {
            ApiService.shared.fetchApiData(urlString: callUri) { (rates: currencyDetailModel?, error: ErrorModel?) in
                if let error = error {
                    spinner.dismissLoader()
                    self.showAlertMessage(titleStr: "Error", messageStr: error.Message!)
                }
                guard let rates = rates else { return }
                self.txtRightInput.text = "\(rates.result)"
                spinner.dismissLoader()
            }
        }
    }
    ...
    func createConvertRatesUri(fromCur: String, date: String, amount: Double, toCur: String) -> String {
        return "\(Routes.convertRatesUri)?from=\(fromCur)&amount=\(amount)&date=\(date)&to=\(toCur)"
    }
